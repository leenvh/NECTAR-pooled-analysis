...
newd=modelbased::estimate_contrasts(model, contrast=c("visit"), by="arm")
newd = newd %>% select(arm,Level1,Level2,Difference,SE)
colnames(newd) = c("arm","compare","ref","coef","se")
...
newd_results <- newd_aligned %>%
  ...
  select(arm, visit = compare_final, RR, se, ci_lower, ci_upper, p_value)
